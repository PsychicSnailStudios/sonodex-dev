                        ...
                        match event.kind {
                            EventKind::Create(_) | EventKind::Modify(_) => {
                                ...
                                let track_opt = {
                                    ...
                                    let try_ampersand =
                                        g("scan_try_parse_ampersand", "true") == "true";

                                    let tag_delim_raw = g("artist_tag_delimiters", " / |; |, |,");
                                    let filename_delim_raw = g(
                                    ...
                                    let tag_delims_ref: Vec<&str> =
                                        tag_delims.iter().map(|s| s.as_str()).collect();
                                    let filename_delims_ref: Vec<&str> =
                                        filename_delims.iter().map(|s| s.as_str()).collect();
                                    let genre_delims_ref: Vec<&str> =
                                        genre_delims.iter().map(|s| s.as_str()).collect();
                                ...
                                match track_opt {
                                    Some(track) => match crate::db::upsert_track(&conn, &track) {
                                        Ok(_) => {
                                    ...
                                    None => {
                                        eprintln!(
                                            "[watcher] read_track returned None for: {:?}",
                                            path
                                        );
                                    }
                                }
