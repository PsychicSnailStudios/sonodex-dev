    ...
    let mut stmt = conn.prepare(
		"SELECT id, uid, path, last_modified, title, artists, album_artist, albums, genres, year, rating, tags, duration_ms, bpm, key, credits, label, artwork_path, user_options
		 FROM (
			 ...
			 UNION
			 SELECT b.id, b.uid, b.path, b.last_modified, b.title, b.artists, b.album_artist, b.albums, b.genres, b.year, b.rating, b.tags, b.duration_ms, b.bpm, b.key, b.credits, b.label, b.artwork_path, b.user_options
			 FROM tracks a
			 INNER JOIN tracks b ON (
				 a.id < b.id
				 AND LOWER(TRIM(a.title)) = LOWER(TRIM(b.title))
