...
pub fn create_playlist(conn: &Connection, playlist: &Playlist) -> Result<()> {
	conn.execute(
		"INSERT INTO playlists (uid, title, description, owner, tracks, artwork_blob, artwork_path)
		 VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)",
		params![
			playlist.uid, playlist.title, playlist.description,
			playlist.owner, playlist.tracks,
			playlist.artwork_blob, playlist.artwork_path
		],
	)?;
	Ok(())
}
...
pub fn get_all_playlists(conn: &Connection) -> Result<Vec<Playlist>> {
	let mut stmt = conn.prepare(
		"SELECT id, uid, title, description, owner, tracks, artwork_path
...
pub fn get_playlist_by_id(conn: &Connection, id: i64) -> Result<Option<Playlist>> {
	let mut stmt = conn.prepare(
		"SELECT id, uid, title, description, owner, tracks, artwork_blob, artwork_path
