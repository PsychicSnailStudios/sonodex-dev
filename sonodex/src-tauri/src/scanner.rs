use crate::db::{
	get_setting, upsert_track, Track,
	Album, AlbumUpdate, create_album, get_all_albums, update_album,
	Artist, ArtistUpdate, create_artist, get_all_artists, update_artist,
};
...
pub fn read_track_with_settings(
	...
	let (
		...
		tag_genres, tag_year, tag_rating, tag_track_number,
		bpm, key, artwork_blob,
	) = if let Some(tag) = tag {
...
fn find_or_create_album(
	...
	if let Some(album) = existing.iter().find(|a| {
		...
		if !existing_tracks.iter().any(|t| t["id"].as_i64() == Some(track_id)) {
			...
			update_album(conn, album_id, &AlbumUpdate {
				format: None,
				title: None,
				rating: None,
				artists: None,
	...
	let album = Album {
		...
		rating: None,
		artists: album_artist.map(|a| {
			serde_json::to_string(&vec![a]).unwrap_or_else(|_| "[]".to_string())
		}),
...
fn find_or_create_artist(
	...
	if let Some(artist) = existing.iter().find(|a| a.name.to_lowercase() == name_lower) {
...
pub fn scan_directory_with_progress(conn: &Connection, dir: &str, app: &AppHandle) {
	let priority_title = get_setting(conn, "filename_priority_title")
		.ok().flatten().unwrap_or_else(|| "tag".to_string());
	...
	let artist_tag_delimiters_owned: Vec<String> = get_setting(conn, "artist_tag_delimiters")
		...
		.unwrap_or_else(|| ARTIST_TAG_DELIMITERS.iter().map(|s| s.to_string()).collect());
	let artist_tag_delimiters: Vec<&str> = artist_tag_delimiters_owned.iter().map(|s| s.as_str()).collect();
