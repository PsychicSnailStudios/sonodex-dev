...
/// Discogs returns year as either a number or a string depending on the endpoint.
fn parse_year(val: &serde_json::Value) -> Option<String> {
	match val {
		...
		serde_json::Value::String(s) => {
			if s.len() == 4 && s.parse::<u32>().is_ok() { Some(s.clone()) } else { None }
		}
		_ => None,
	}
}

async fn fetch_artwork(client: &Client, url: &str) -> Option<Vec<u8>> {
...
pub async fn search(
	...
	// The search result gives us basic fields. If we have a resource_url,
	// fetch the full release to get labels, formats, and full images.
	...
	// Parse the title field — Discogs search returns "Artist - Album" in title.
	let (album_name, artist_name) = if let Some(ref t) = result.title {
		if let Some(pos) = t.find(" - ") {
			(Some(t[pos + 3..].trim().to_string()), Some(t[..pos].trim().to_string()))
		} else {
			(Some(t.clone()), None)
		}
	} else {
	...
	let artists = artist_name.as_deref()
		.or_else(|| full.as_ref().and_then(|f| f.artists.as_ref()?.first()?.name.as_deref()))
		.map(|a| serde_json::to_string(&vec![a.to_string()]).unwrap_or_else(|_| "[]".to_string()));

	let album_artist = artist_name.clone()
		.or_else(|| full.as_ref().and_then(|f| f.artists.as_ref()?.first()?.name.clone()));
	...
	let year = result.year.as_ref().and_then(parse_year)
		.or_else(|| full.as_ref().and_then(|f| f.year.map(|y| y.to_string())));

	let genres = full.as_ref()
