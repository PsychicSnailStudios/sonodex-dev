...
pub async fn enrich_artist_async(
	...
	if let Some(r) = lastfm_result {
		if merged.about.is_none() { merged.about = r.about; }
		if merged.genres.is_none() { merged.genres = r.genres; }
		if merged.websites.is_none() { merged.websites = r.websites; }
		if merged.profile_art.is_none() { merged.profile_art = r.profile_art; }
	}
