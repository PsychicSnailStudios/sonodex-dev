...
use crate::db::{Track, Playlist, PlaylistTrackEntry};
...
pub async fn spotify_get_playlists(profile_uid: &str) -> Result<Vec<SpotifyPlaylistSummary>, String> {
...
pub async fn enrich_track(
	...
	title: &str,
	artist: &str,
) -> Result<Option<SpotifyTrackMeta>, String> {
	let token = get_valid_token(profile_uid).await?;
	let client = Client::new();
	...
	let body: SearchResult = res.json().await.map_err(|e| e.to_string())?;
	let Some(item) = body.tracks.items.into_iter().next() else {
		return Ok(None);
	};
	...
	Ok(Some(SpotifyTrackMeta {
		...
		isrc: item.external_ids.and_then(|e| e.isrc),
	}))
}
...
pub async fn enrich_album(
	...
	title: &str,
	artist: &str,
) -> Result<Option<SpotifyAlbumMeta>, String> {
	let token = get_valid_token(profile_uid).await?;
	let client = Client::new();
	...
	let body: SearchResult = res.json().await.map_err(|e| e.to_string())?;
	let Some(item) = body.albums.items.into_iter().next() else {
		return Ok(None);
	};
	...
	let (genres, label) = if full_res.status().is_success() {
		let full: SpotifyAlbumFull = full_res.json().await.map_err(|e| e.to_string())?;
		(full.genres, full.label)
	} else {
		(None, None)
	};
...
pub async fn enrich_artist(
	profile_uid: &str,
	name: &str,
) -> Result<Option<SpotifyArtistMeta>, String> {
	let token = get_valid_token(profile_uid).await?;
	let client = Client::new();
	...
	let body: SearchResult = res.json().await.map_err(|e| e.to_string())?;
	let Some(item) = body.artists.items.into_iter().next() else {
		return Ok(None);
	};
